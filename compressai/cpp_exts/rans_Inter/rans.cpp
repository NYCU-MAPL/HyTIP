...
int MLCodec_RansEncoder_InterLib::add_cdf(
...
void MLCodec_RansEncoder_InterLib::empty_cdf_buffer() {
  _ransSymbols.clear();
  _cdfs_sizes.clear();
  _offsets.clear();
}

void MLCodec_RansEncoder_InterLib::encode_with_indexes(
...
void MLCodec_RansEncoder_InterLib::flush() {
  RansState rans;
  RansEncInit(&rans);
...
std::vector<uint8_t> MLCodec_RansEncoder_InterLib::get_encoded_stream() { return _stream; }

void MLCodec_RansEncoder_InterLib::reset() { _syms.clear(); }

MLCodec_RansEncoder_InterLibMultiThread::MLCodec_RansEncoder_InterLibMultiThread()
    : MLCodec_RansEncoder_InterLib(), m_finish(false), m_result_ready(false),
      m_thread(std::thread(&MLCodec_RansEncoder_InterLibMultiThread::worker, this)) {}
...
void MLCodec_RansEncoder_InterLibMultiThread::encode_with_indexes(
    const std::shared_ptr<std::vector<int16_t>> symbols,
    const std::shared_ptr<std::vector<int16_t>> indexes,
    const int cdf_group_index) {
  PendingTask p;
...
void MLCodec_RansEncoder_InterLibMultiThread::flush() {
  PendingTask p;
...
std::vector<uint8_t> MLCodec_RansEncoder_InterLibMultiThread::get_encoded_stream() {
  std::unique_lock<std::mutex> lk(m_mutex_result);
  m_cv_result.wait(lk, [this] { return m_result_ready || m_finish; });
  return MLCodec_RansEncoder_InterLib::get_encoded_stream();
}

void MLCodec_RansEncoder_InterLibMultiThread::reset() {
  MLCodec_RansEncoder_InterLib::reset();
  std::lock_guard<std::mutex> lk(m_mutex_result);
  m_result_ready = false;
}

void MLCodec_RansEncoder_InterLibMultiThread::worker() {
  while (!m_finish) {
    std::unique_lock<std::mutex> lk(m_mutex_pending);
    ...
    while (m_pending.size() > 0) {
      ...
      if (p.workType == WorkType::Encode) {
        MLCodec_RansEncoder_InterLib::encode_with_indexes(p.symbols, p.indexes,
                                            p.cdf_group_index);
      } else if (p.workType == WorkType::Flush) {
        MLCodec_RansEncoder_InterLib::flush();
        {
...
void MLCodec_RansDecoder_InterLib::set_stream(
...
int MLCodec_RansDecoder_InterLib::add_cdf(
    const std::shared_ptr<std::vector<std::vector<int32_t>>> cdfs,
    const std::shared_ptr<std::vector<int32_t>> cdfs_sizes,
    const std::shared_ptr<std::vector<int32_t>> offsets) {
...
void MLCodec_RansDecoder_InterLib::empty_cdf_buffer() {
  _cdfs.clear();
  _cdfs_sizes.clear();
  _offsets.clear();
}

std::vector<int16_t> MLCodec_RansDecoder_InterLib::decode_stream(
    const std::shared_ptr<std::vector<int16_t>> indexes,
    const int cdf_group_index) {
