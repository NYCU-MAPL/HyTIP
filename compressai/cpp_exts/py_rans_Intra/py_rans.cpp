...
MLCodec_RansEncoder_Intra::MLCodec_RansEncoder_Intra(bool multiThread, int streamPart = 1) {
  bool useMultiThread = multiThread || streamPart > 1;
  for (int i = 0; i < streamPart; i++) {
    if (useMultiThread) {
      m_encoders.push_back(std::make_shared<MLCodec_RansEncoder_IntraLibMultiThread>());
    } else {
      m_encoders.push_back(std::make_shared<MLCodec_RansEncoder_IntraLib>());
    }
  }
}

void MLCodec_RansEncoder_Intra::encode_with_indexes(const py::array_t<int16_t> &symbols,
...
void MLCodec_RansEncoder_Intra::flush() {
  for (auto encoder : m_encoders) {
    encoder->flush();
  }
}

py::array_t<uint8_t> MLCodec_RansEncoder_Intra::get_encoded_stream() {
...
void MLCodec_RansEncoder_Intra::reset() {
  for (auto encoder : m_encoders) {
    encoder->reset();
  }
}

MLCodec_RansDecoder_Intra::MLCodec_RansDecoder_Intra(int streamPart) {
  for (int i = 0; i < streamPart; i++) {
    m_decoders.push_back(std::make_shared<MLCodec_RansDecoder_IntraLib>());
  }
}

void MLCodec_RansDecoder_Intra::set_stream(const py::array_t<uint8_t> &encoded) {
...
py::array_t<int16_t>
MLCodec_RansDecoder_Intra::decode_stream(const py::array_t<int16_t> &indexes,
                           const py::array_t<int32_t> &cdfs,
                           const py::array_t<int32_t> &cdfs_sizes,
                           const py::array_t<int32_t> &offsets) {
  py::buffer_info indexes_buf = indexes.request();
...
PYBIND11_MODULE(MLCodec_rans_Intra, m) {
  m.attr("__name__") = "compressai.MLCodec_rans_Intra";

  m.doc() = "range Asymmetric Numeral System python bindings";

  py::class_<MLCodec_RansEncoder_Intra>(m, "MLCodec_RansEncoder_Intra")
      .def(py::init<bool, int>())
      .def("encode_with_indexes", &MLCodec_RansEncoder_Intra::encode_with_indexes)
      .def("flush", &MLCodec_RansEncoder_Intra::flush)
      .def("get_encoded_stream", &MLCodec_RansEncoder_Intra::get_encoded_stream)
      .def("reset", &MLCodec_RansEncoder_Intra::reset);

  py::class_<MLCodec_RansDecoder_Intra>(m, "MLCodec_RansDecoder_Intra")
      .def(py::init<int>())
      .def("set_stream", &MLCodec_RansDecoder_Intra::set_stream)
      .def("decode_stream", &MLCodec_RansDecoder_Intra::decode_stream);
}
