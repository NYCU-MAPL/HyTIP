...
void MLCodec_RansEncoder_IntraLib::encode_with_indexes(
...
void MLCodec_RansEncoder_IntraLib::flush() {
  Rans64State rans;
  Rans64EncInit(&rans);
...
std::vector<uint8_t> MLCodec_RansEncoder_IntraLib::get_encoded_stream() { return _stream; }

void MLCodec_RansEncoder_IntraLib::reset() { _syms.clear(); }

MLCodec_RansEncoder_IntraLibMultiThread::MLCodec_RansEncoder_IntraLibMultiThread()
    : MLCodec_RansEncoder_IntraLib(), m_finish(false), m_result_ready(false),
      m_thread(std::thread(&MLCodec_RansEncoder_IntraLibMultiThread::worker, this)) {}
...
void MLCodec_RansEncoder_IntraLibMultiThread::encode_with_indexes(
    const std::shared_ptr<std::vector<int16_t>> symbols,
    const std::shared_ptr<std::vector<int16_t>> indexes,
    const std::shared_ptr<std::vector<std::vector<int32_t>>> cdfs,
    const std::shared_ptr<std::vector<int32_t>> cdfs_sizes,
...
void MLCodec_RansEncoder_IntraLibMultiThread::flush() {
  PendingTask p;
...
std::vector<uint8_t> MLCodec_RansEncoder_IntraLibMultiThread::get_encoded_stream() {
  std::unique_lock<std::mutex> lk(m_mutex_result);
  m_cv_result.wait(lk, [this] { return m_result_ready || m_finish; });
  return MLCodec_RansEncoder_IntraLib::get_encoded_stream();
}

void MLCodec_RansEncoder_IntraLibMultiThread::reset() {
  MLCodec_RansEncoder_IntraLib::reset();
  std::lock_guard<std::mutex> lk(m_mutex_result);
  m_result_ready = false;
}

void MLCodec_RansEncoder_IntraLibMultiThread::worker() {
  while (!m_finish) {
    std::unique_lock<std::mutex> lk(m_mutex_pending);
    ...
    while (m_pending.size() > 0) {
      ...
      if (p.workType == WorkType::Encode) {
        MLCodec_RansEncoder_IntraLib::encode_with_indexes(p.symbols, p.indexes, p.cdfs,
                                            p.cdfs_sizes, p.offsets);
      } else if (p.workType == WorkType::Flush) {
        MLCodec_RansEncoder_IntraLib::flush();
        {
...
void MLCodec_RansDecoder_IntraLib::set_stream(
...
std::vector<int16_t> MLCodec_RansDecoder_IntraLib::decode_stream(
    const std::shared_ptr<std::vector<int16_t>> indexes,
    const std::shared_ptr<std::vector<std::vector<int32_t>>> cdfs,
    const std::shared_ptr<std::vector<int32_t>> cdfs_sizes,
    const std::shared_ptr<std::vector<int32_t>> offsets) {
