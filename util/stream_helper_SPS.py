...
def read_uint_adaptive_with_bytes(f):
    a3 = read_uchars(f, 1)[0]
    a2 = read_uchars(f, 1)[0]
...
def write_ip_segment(f, is_i_frame, sps_id, strings: list[bytes]):
    written = 0
    written_list = []
    flag = (int(NalType.NAL_I if is_i_frame else NalType.NAL_P) << 4) + sps_id
    single_len = write_uchars(f, (flag,))
    written_list.append(single_len)
    written += single_len
    # we write all the streams in the same file, thus, we need to write the per-frame length
    # if packed independently, we do not need to write it
    for i, bit_stream in enumerate(strings):
        single_len = write_uint_adaptive(f, len(bit_stream))
        single_len += write_bytes(f, bit_stream)
        written_list.append(single_len)
        written += single_len
    return written, written_list
...
def read_ip_remaining_segment(f, read_byte_segment: int):
    strings = []
    num_bytes_list = []
    for _ in range(read_byte_segment):
        stream_length, num_bytes = read_uint_adaptive_with_bytes(f)
        num_bytes_list.append(num_bytes)
        bit_stream = read_bytes(f, stream_length)
        strings.append(bit_stream)
    return strings, num_bytes_list
...
def flatten_strings_list(strings_list):
    for item in strings_list:
        if isinstance(item, list):
            yield from flatten_strings_list(item)
        else:
            yield item
