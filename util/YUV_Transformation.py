...
def rgb2ycbcr420(rgb: Tensor) -> Tensor:
    """RGB to YCbCr conversion for torch Tensor.
    Using ITU-R BT.709 coefficients.
    ...
    # to 420
    cb = torch.mean(torch.reshape(cb, (seq, 1, h//2, 2, w//2, 2)), dim=(-1, -3))
    cr = torch.mean(torch.reshape(cr, (seq, 1, h//2, 2, w//2, 2)), dim=(-1, -3))
    uv = torch.cat((cb, cr), dim=1)
...
def RGB2YUV420_v2(RGB):
    YUV = RGB2YUV_v2(RGB)
    return YUV[:, 0:1], YUV[:, 1:, ::2, ::2]
